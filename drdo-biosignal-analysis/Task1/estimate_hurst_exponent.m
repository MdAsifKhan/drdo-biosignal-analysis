function [hurst] = estimate_hurst_exponent(inputdata)   

data=inputdata;         % make a local copy
[~,npoints]=size(inputdata);
yvals=zeros(1,npoints);xvals=zeros(1,npoints);
data2=zeros(1,npoints);index=0;
binsize=1;
while npoints>4
    y=std(data);
    index=index+1;
